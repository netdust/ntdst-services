...
class Select
{
	...
	{
		$multiple = $field->addon('multiple')
			? 'multiple="multiple"'
			: '';
		$name = $field->addon('multiple')
			? $field->inputName() . '[]'
			: $field->inputName();
		$pretty = $field->addon('pretty')
			? 'pretty-select'
			: '';

		echo '<select ' . esc_attr($multiple) . ' name="' . esc_attr($name) . '" id="' . esc_attr(
			$field->inputId()
		) . '" class="' . esc_attr($pretty) . '">';

		$options = is_callable($field->addon('options'))
			? $field->addon('options')()
			: $field->addon('options');
		...
		foreach ($options as $optionValue => $optionLabel) {
			...
			echo '<option value="' . esc_attr($optionValue) . '" ' . $selected . '>' . esc_html(
				$optionLabel
			) . '</option>';
		}
